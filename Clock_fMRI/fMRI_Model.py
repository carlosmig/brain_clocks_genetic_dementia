# ============================================================
# fMRI FC Brain Age Pipeline
# - Train SVR(poly=2) on HCs with repeated 5-fold CV
...
# 0) Load FC dataframe
# ----------------------------
DATA_CSV = r"fMRI_data.csv"
df = pd.read_csv(DATA_CSV)

# Expect columns: Age, Sex, Diagnosis, Region, FC_*
fc_cols = [c for c in df.columns if c.startswith('FC_')]
if len(fc_cols) == 0:
    raise ValueError("No FC_ columns found in fMRI_data.csv.")

# Features & targets
X_all = np.nan_to_num(df[fc_cols].to_numpy(), nan=0.0)
...
plt.title('fMRI FC model: Real vs Predicted Age (HC-trained)')
...
plt.title("fMRI FC Brain Age Gaps (Adjusted for Age & Sex)\nOutliers removed (±2 SD)\n(AD & HCs restricted to North)")
